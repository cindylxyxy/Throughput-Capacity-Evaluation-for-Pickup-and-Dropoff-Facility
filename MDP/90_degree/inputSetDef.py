...
def in_Hout(j, k, m, n):
	...
	if (angle == 90):
		assert (m_in > m_out)
		if (J + dgap >= K >= J - dgap) or (spot2blk(j) + 1 == spot2blk(k)):
...
########################################################################################################################################################
# [MDP specific version] Please refer to the file with the same name under the folder access_control/ for the original version
...
def tau(x, y, i, j):
	for j_ in range(j - 1, 0, -1):
		if 2 + m_out <= x[j_-1] <= m_in + m_out and spot2blk(j_) > i and (m_in - (x[j_-1] - m_out - 1)) + (spot2blk(j) - (spot2blk(j_) - 1)) >= g_out:
			return False
	for i_ in range(spot2blk(j), i, -1):
		if y[i_-1] == j:
			assert i_ <= spot2in(j)
			# we can directly return a True value here because the check started with spots of larger indices. 
			return True
...
def gamma(x, y, j):
	...
	n = len(y)
	N = len(x)
	assert n == spot2blk(N) + 1 or (angle == 0 and mode == 'long')
	...
	while blk2blk(i_) <= n and y[blk2blk(i_) - 1] > 0:
		# [MDP specific change] y-vector can now take N + 2 for replacement vehicles not yet assigned a spot 
	...
	if m_in > m_out: 
		...
		# so far, we have found the block i_ s.t. all blocks between itself and i(j)+ (inclusive) are all occupied, while the block immediately ahead of itself is empty.
		# in other words, the lead vehicle of the platoon passing the block i(j)+ is in block i_.  

		if blk2blk(blk2blk(i_)) <= n:

			if (1 <= y[blk2blk(blk2blk(i_)) - 1] <= N) and (spot2in(y[blk2blk(blk2blk(i_)) - 1]) == blk2blk(blk2blk(i_))):
	...
	if m_in <= m_out:
		...
		if (1 <= y[i_ - 1] <= N) and spot2in(y[i_ - 1]) == i_ and 0 <= x[y[i_-1] - 1] - 2 <= m_in - 2:
			# i.e. if the vehicle in i_ is planning to enter the spot y[i_] s.t. i(y[i_]) == i_ and
			# 1) the spot y[i_] is empty => no issue raised as m_in <= m_out
			# 2) the spot y[i_] has an exiting vehicle 
			#    => if the completed steps m is s.t. m >= m_in - 1, the remaining number of steps is m_out - m <= m_out - (m_in - 1)
			#       in the last step, the replacement vehicle in i_ can start the enter maneuver
			#       thus in at most m_out - (m_in - 1) + m_in - 1 = m_out steps, the replacement vehicle finishes the enter maneuver
			#       simultaneously, the vehicle in block i__ s.t. (i__)+ = i_ can move forward
			return False

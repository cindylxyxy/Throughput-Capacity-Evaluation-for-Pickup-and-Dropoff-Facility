...
class event():
	...
	def __eq__(self, other):
		return (float(self.time) == float(other.time)) & (self.typ == other.typ) & (self.vehicle == other.vehicle) 

	def __gt__(self, other):
		if float(self.time) == float(other.time):
			if (self.typ == other.typ):
				if (self.vehicle is None) | (other.vehicle is None):
					assert (self.vehicle is None) and (other.vehicle is None)
